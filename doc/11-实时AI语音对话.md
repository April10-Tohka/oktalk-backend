下面是为你这个 **Free Talk（实时语音对话）模块** 定制的一份**可直接用于毕设/开发的PRD**。我会按“产品 + 技术实现 + 架构约束”三层来写，确保你不仅能写文档，还能直接落地代码。

---

# 📄 PRD：AI英语学习App - Free Talk 实时语音对话模块
...
utteranceChan := make(chan []byte, 10)
```
...
```text
连接 → run-task → 发送音频 → finish-task → 获取结果 → 关闭连接
```
