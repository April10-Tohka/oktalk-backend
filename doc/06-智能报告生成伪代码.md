...
## 一、接口：POST /api/v1/report/MVP

**功能**：同步生成阶段性学习报告（基于 MVP，实现完整 JSON 报告结构）

**流程类型**：同步 API（查询数据 → 统计分析 → LLM 生成报告 → 保存数据库 → 返回 JSON）
...
```
流程：POST /api/v1/report/MVP
├─ 【同步阶段】
│  ├─ 步骤 1：接收 HTTP 请求
│  │  ├─ 请求体参数（ReportMVPRequest）：
│  │  │  ├─ report_type（字符串，必填）：["weekly", "monthly", "custom"]
│  │  │  ├─ start_date（字符串 YYYY-MM-DD，自定义时必填）
│  │  │  ├─ end_date（字符串 YYYY-MM-DD，自定义时必填）
│  │  │  └─ 由中间件注入 user_id（当前登录用户）
...
│  │
│  ├─ 步骤 4：参数校验
│  │  ├─ 检查 report_type 是否为 ["weekly","monthly","custom"]
│  │  │  └─ 异常点：非法值 → 返回 400
│  │  ├─ 如果 report_type == "custom"：
│  │  │  ├─ 检查 start_date / end_date 不为空且格式为 YYYY-MM-DD
│  │  │  ├─ 检查 start_date <= end_date
│  │  │  └─ 检查日期范围 ≤ 1 年
│  │  └─ 异常点：参数格式或范围错误 → 返回 400
│  │
│  ├─ 步骤 5：确定统计周期（周期起止日期）
│  │  ├─ 如果 report_type == "weekly"：
│  │  │  ├─ period_start_date = 当前日期所在周的周一
│  │  │  └─ period_end_date = 当前日期所在周的周日
│  │  ├─ 如果 report_type == "monthly"：
│  │  │  ├─ period_start_date = 当前月第一天
│  │  │  └─ period_end_date = 当前月最后一天
│  │  └─ 如果 report_type == "custom"：
│  │     ├─ period_start_date = start_date
│  │     └─ period_end_date = end_date
│  │
│  ├─ 步骤 6：统计学习数据（当前周期）
│  │  ├─ 从 voice_conversations 统计：
│  │  │  ├─ total_conversations = COUNT(id)
│  │  │  ├─ total_study_minutes = SUM(duration_seconds) / 60
│  │  │  └─ active_days = COUNT(DISTINCT DATE(created_at))
│  │  ├─ 从 pronunciation_evaluations 统计：
│  │  │  ├─ total_evaluations = COUNT(id)
│  │  │  ├─ average_evaluation_score = AVG(overall_score)
│  │  │  ├─ average_accuracy_score = AVG(accuracy_score)
│  │  │  ├─ average_fluency_score = AVG(fluency_score)
│  │  │  ├─ average_integrity_score = AVG(integrity_score)
│  │  │  ├─ s_level_count = COUNT(feedback_level='S')
│  │  │  ├─ a_level_count = COUNT(feedback_level='A')
│  │  │  ├─ b_level_count = COUNT(feedback_level='B')
│  │  │  └─ c_level_count = COUNT(feedback_level='C')
│  │  ├─ 聚合问题单词：
│  │  │  ├─ 从 problem_words 字段汇总所有单词
│  │  │  └─ 统计出现频次，取前 3-5 个高频难词
│  │  └─ 异常点：数据库查询失败 → 返回 500 Internal Server Error
│  │
│  ├─ 步骤 7：统计上一周期数据（用于进步对比）
│  │  ├─ 根据 report_type 计算上一周期的 start/end（上周 / 上月 / 上一自定义周期）
│  │  ├─ 从 pronunciation_evaluations 统计：
│  │  │  ├─ previous_avg_score = 上一周期 AVG(overall_score)
│  │  │  └─ previous_sa_count = 上一周期 S/A 级评测数
│  │  └─ 如果上一周期没有数据：
│  │     ├─ previous_avg_score = 0
│  │     └─ 进步率等字段使用默认值（例如 0）
│  │
│  ├─ 步骤 8：构建 ReportStats 统计对象
│  │  ├─ stats = {
│  │  │    user_id,
...
│  │  │    active_days,
│  │  │    avg_accuracy: average_accuracy_score,
│  │  │    avg_fluency: average_fluency_score,
│  │  │    avg_integrity: average_integrity_score,
│  │  │    current_avg_score: average_evaluation_score,
│  │  │    previous_avg_score,
│  │  │    sa_level_count: s_level_count + a_level_count,
│  │  │    previous_sa_level_count: previous_sa_count,
│  │  │    difficult_words: 高频问题单词列表
│  │  │  }
│  │
│  ├─ 步骤 9：生成 LLM Prompt 并调用 LLM 生成报告文案
│  │  ├─ prompt = GenerateReportPrompt(stats)
│  │  ├─ 调用 LLMProvider.Chat(ctx, prompt, "")
│  │  │  ├─ 返回：report_text（包含鼓励语、亮点、建议等）
│  │  │  └─ 异常处理：LLM 调用失败 → 返回 500 或使用简单模板 fallback
│  │
│  ├─ 步骤 10：根据 report_text 和 stats 构建 ReportMVPResponse
│  │  ├─ 解析 / 组织 LLM 输出，将其映射到：
│  │  │  ├─ ActivityStats（对话次数、评测次数、总时长、活跃天数）
│  │  │  ├─ AbilityRadar（accuracy/fluency/integrity + 简要总结）
│  │  │  ├─ ProgressStats（current_score、previous_score、overall_score_change、highlights）
│  │  │  ├─ KidFriendlyCard（鼓励语 + 本周亮点 + 小目标）
│  │  │  ├─ DifficultWords（高频问题单词 + 示范音频 URL，如有）
│  │  │  └─ FullReport（完整报告文本 full_text + 各段摘要）
│  │  ├─ 生成 report_id（UUID）
│  │  ├─ 构造 ReportMVPResponse:
│  │  │  ├─ report_id, report_type, period_start_date, period_end_date
│  │  │  ├─ activity_stats, ability_radar, progress_stats
│  │  │  ├─ kid_friendly_card, difficult_words, full_report
│  │  └─ 异常点：解析 report_text 失败 → 使用默认模板填充关键字段
│  │
│  ├─ 步骤 11：保存报告到数据库 learning_reports
│  │  ├─ 构建 learning_reports 记录：
│  │  │  ├─ id = report_id（UUID）
│  │  │  ├─ user_id
│  │  │  ├─ report_type
│  │  │  ├─ period_start_date / period_end_date
│  │  │  ├─ total_conversations / total_evaluations / total_study_minutes
│  │  │  ├─ average_conversation_score（如暂缺，可为 0）
│  │  │  ├─ average_evaluation_score / average_accuracy_score / average_fluency_score / average_integrity_score
│  │  │  ├─ s_level_count / a_level_count / b_level_count / c_level_count
│  │  │  ├─ improvement_rate（(current_avg_score - previous_avg_score) / previous_avg_score，上一期为 0 时设为 0）
│  │  │  ├─ most_practiced_topics / problem_words（JSON）
│  │  │  ├─ strengths / weaknesses / recommendations（从 LLM 输出提取）
│  │  │  ├─ generated_by = "llm_mvp"
│  │  │  ├─ created_at = now
│  │  │  └─ updated_at = now
│  │  ├─ 调用 Database.InsertLearningReport(记录)
│  │  └─ 异常处理：数据库写入失败 → 记录日志，但仍返回报告给前端
│  │
│  ├─ 步骤 12：返回响应
│  │  ├─ code = 200
│  │  ├─ message = "success"
│  │  └─ data = ReportMVPResponse
│  │
│  └─ 流程结束
│
└─ 整体结束
...
│  ├─ 步骤 1：接收 HTTP 请求
│  │  ├─ 请求体参数：
│  │  │  ├─ report_type（字符串）：["weekly", "monthly"]
...
│  │  │  }
│  │  ├─ 调用 WorkerManager.SubmitTask(ctx, task)
│  │  │  └─ 异常点：队列满或提交失败 → 返回 503
│  │  └─ 提交成功后，任务状态在 TaskCache 中初始化为 pending
│  │
│  ├─ 步骤 12：构建响应体
│  │  ├─ 响应结构：
│  │  │  ├─ code = 200
...
│  │
│  ├─ Worker goroutine 从任务队列中拉取 report 任务
│  │  ├─ 调用 ReportTaskProcessor.Process(ctx, task)
│  │  └─  结果对象 result 和 result_key = report:result:{report_id}
│  │
│  ├─ 步骤 A1：查询评测数据
...
│  │
│  ├─ 步骤 A3：计算评测统计
│  │  ├─ 更新报告状态： current_stage = "analyzing_eval"
│  │  ├─ 构建评测分析对象：
│  │  │  ├─ 总体统计：
...
│  │
│  ├─ 步骤 A4：计算对话统计
│  │  ├─ 更新报告状态： current_stage = "analyzing_chat"
│  │  ├─ 构建对话分析对象：
│  │  │  ├─ 总体统计：
...
│  │
│  ├─ 步骤 A5：计算学习进度
│  │  ├─ 更新报告状态： current_stage = "calculating_progress"
│  │  ├─ 构建学习进度对象：
│  │  │  ├─ 总体进度：
...
│  │
│  ├─ 步骤 A6：生成发音改善分析
│  │  ├─ 更新报告状态： current_stage = "analyzing_improvement"
│  │  ├─ 构建改善分析对象：
│  │  │  ├─ 改善最明显的方面：
│  │  │  │  ├─ 对比前后时期的分数变化
...
│  │
│  ├─ 步骤 A7：生成学习建议
│  │  ├─ 更新报告状态： current_stage = "generating_recommendations"
│  │  ├─ 构建建议对象：
│  │  │  ├─ 针对困难音素的建议：
│  │  │  │  ├─ 对每个困难音素（score < 70）：
│  │  │  │  │  ├─ 查询包含该音素的相关学习文本
...
│  │
│  ├─ 步骤 A8：生成可视化数据
│  │  ├─ 更新报告状态： current_stage = "generating_charts"
│  │  ├─ 构建可视化对象：
│  │  │  ├─ 分数趋势图：
│  │  │  │  ├─ x_axis = date（日期）
...
## 二、接口：GET /api/v1/report/{report_id}
...
│  │
│  ├─ 步骤 4：路径参数校验
│  │  ├─ 检查 report_id 格式（例如 UUID 或 report_xxxxx）
│  │  └─ 异常点：格式不合法 → 返回 400
│  │
│  ├─ 步骤 5：从 TaskCache 查询报告任务状态
│  │  ├─ 调用 Cache.GetTaskStatus(report_id)
│  │  ├─ 返回：task_meta（包含 status、user_id、result_key 等）
│  │  └─ 异常点：
│  │     ├─ Redis 连接失败 → 降级到数据库
│  │     └─ 缓存未命中 → 继续步骤 6
│  │
│  ├─ 步骤 6：权限检查
│  │  ├─ 如果 task_meta 存在：
│  │  │  ├─ 检查 task_meta.user_id == 当前 user_id
│  │  │  └─ 否则 → 返回 403 Forbidden
│  │  └─ 如果 task_meta 不存在：
│  │     ├─ 调用 Database.GetLearningReport(report_id)
│  │     ├─ 异常点：
│  │     │  ├─ 报告不存在 → 返回 404 Not Found
│  │     │  └─ report_record.user_id != 当前 user_id → 返回 403 Forbidden
│  │     └─ 找到记录 → 视为 status="success"，后续直接返回 DB 内容
│  │
│  ├─ 步骤 7：根据任务状态返回不同响应
│  │  │
│  │  ├─ 情况 A：status == "pending"
...
│  │  │  │    }
│  │  │
│  │  ├─ 情况 B：status == "processing"
│  │  │  ├─ （可选）从 task_meta 读取   current_stage
│  │  │  ├─ 返回：
│  │  │  │  ├─ code = 200
│  │  │  │  ├─ message = "processing"
│  │  │  │  └─ data = {
│  │  │  │      report_id,
│  │  │  │      status: "processing",
│  │  │  │      current_stage: task_meta.current_stage（可选）
│  │  │  │    }
│  │  │
│  │  ├─ 情况 C：status == "failed"
│  │  │  ├─ 返回：
...
│  │  │  │      report_id,
│  │  │  │      status: "failed",
│  │  │  │      error_message: task_meta.error,
│  │  │  │      message: "报告生成失败，请稍后重试"
│  │  │  │    }
│  │  │
│  │  ├─ 情况 D：status == "success"
│  │  │  ├─ 如果 task_meta 存在且带有 result_key：
│  │  │  │  ├─ 调用 ReportCache.Get(result_key) // report:result:{report_id}
│  │  │  │  ├─ 命中 → 得到 ReportResponse
│  │  │  │  └─ 未命中 → 调用 Database.GetLearningReport(report_id) 映射为响应结构
│  │  │  ├─ 如果 task_meta 不存在（只在 DB 中存在历史报告）：
│  │  │  │  ├─ 直接使用 Database.GetLearningReport(report_id) 的内容映射为响应结构
│  │  │  ├─ 返回：
│  │  │  │  ├─ code = 200
│  │  │  │  ├─ message = "success"
│  │  │  │  └─ data = ReportMVPResponse（或从 DB 映射出的等价结构）
│  │  │
│  │  └─ 其他情况（未知 status）
│  │     ├─ 记录错误日志
