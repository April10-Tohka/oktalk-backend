...
**用户体验优先的分层响应策略**:
```
用户请求 → 立即返回基础评分 (2-3s) → 后台异步生成反馈 (3-4s) → 前端不断轮询获取完整结果
```
...
#### 示例:
```
oktalk:eval:result:eval_67890              # 发音评测完整结果
oktalk:demo:audio:word:apples              # 单词示范音频URL
oktalk:demo:audio:sentence:i_like_apples   # 句子示范音频URL
oktalk:user:quota:user_12345:20260122      # 用户每日评测配额
oktalk:feedback:text:78:apples:B           # 基于分数+问题词的通用反馈文本
oktalk:feedback:text:eval_67890            # 某次评测的反馈文本
oktalk:user:stats:user_12345               # 用户学习统计（用于学习报告）
oktalk:session:sess_123                    # 用户登录会话（用于语音对话/评测鉴权）
```
...
**Key**: `oktalk:eval:status:{task_id}`  
**Type**: String  
**Value**: `"pending"` | `"completed"` | `"failed"`  | `"processing"`
**TTL**: 1天 (604800秒)

**示例**:
```redis
SET oktalk:eval:status:task_67890 "pending" EX 604800
```
...
**示例**:
```redis
SET oktalk:user:quota:user_12345:20260122 "15" EX <当天剩余秒数>
...
**示例**:
```redis
SET oktalk:feedback:audio:eval_67890
    "https://cdn.oktalk.com/feedback/eval_67890.mp3"
    EX 604800
```

**使用场景**:
- 缓存某次评测生成的反馈语音URL，避免重复生成TTS
- 支持用户在学习报告或历史记录中反复播放反馈音频

---

#### 2.2.7 用户信息与学习统计缓存 (JSON)
...
**Type**: String (JSON)  
**TTL**: 用户信息 1小时，用户统计 5分钟

**示例**:
```redis
SET oktalk:user:profile:user_12345
    '{"id":"user_12345","nickname":"Tom","age":8}'
    EX 3600

SET oktalk:user:stats:user_12345
    '{"total_evaluations":10,"total_conversations":5,"average_score":82.5}'
    EX 300
```

**使用场景**:
- 减少获取用户信息、学习统计时的数据库查询
- 为学习报告生成和学习面板提供快速统计数据（功能 3）

---

#### 2.2.8 会话与 Token 缓存 (String / JSON)
...
**Type**: String / JSON  
**TTL**: Token 与登录有效期一致，会话默认 24 小时

**示例**:
```redis
SET oktalk:user:token:user_12345 "jwt_token_xxx" EX 7200

SET oktalk:session:sess_123
    '{"session_id":"sess_123","user_id":"user_12345","device_type":"mobile"}'
    EX 86400
```

**使用场景**:
- 缓存用户登录态，支持 AI 语音对话与发音评测接口鉴权（功能 1 / 功能 2）
- 支持多端登录场景下的会话管理与踢端逻辑
...
| 数据类型 | TTL | 过期策略 | 理由 |
|---------|-----|---------|------|
| 发音评测结果 | 7天 | 固定过期 | 用户可能一周内多次查看（功能 2） |
| 示范音频URL | 30天 | 固定过期 | 高频词汇长期有效（功能 2 示范音频） |
| 用户配额 | 当天结束 | 动态计算 | 每日限额自然重置（限制评测次数） |
| 上传令牌 | 5分钟 | 固定过期 | 安全性要求 |
| LLM文本缓存 | 7天 | 固定过期 | 相似场景复用（分级反馈） |
| 反馈音频URL | 7天 | 固定过期 | 反馈语音可反复播放 |
| 用户信息 | 1小时 | 固定过期 | 降低用户信息查询开销 |
| 用户学习统计 | 5分钟 | 固定过期 | 支撑学习报告/面板（功能 3） |
| 会话Session | 24小时 | 固定过期 | 支撑语音对话/评测鉴权（功能 1/2） |
| 用户Token | 与登录有效期一致 | 固定过期 | 控制登录态有效期 |
