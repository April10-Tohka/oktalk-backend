...
├─ 【同步阶段】
│  ├─ 步骤 1：接收 HTTP 请求
│  │  ├─ 参数：audio_file（文件，必填）、text_id（字符串，必填）、category（字符串，必填）、difficulty_level（字符串，可选）
│  │
│  ├─ 步骤 2：身份验证
...
│  │  ├─ 检查 audio_file 是否为空
│  │  │  └─ 异常点：为空 → 返回 400
│  │  ├─ 检查 category 是否为 ["read_word", "read_sentence"]
│  │  │  └─ 异常点：非法格式 → 返回 400
...
│  │
│  ├─ 步骤 6：查询朗读文本
│  │  ├─ 调用Cache.GetTextByID(text_id)
│  │  │  ├─ 返回：text_record（包含 content、category 等）
│  │  │  └─ 
│  │  ├─ 否则（text_id 不存在于缓存）：
...
│  ├─ 步骤 9：序列化任务 Payload
│  │  ├─ payload = {
│  │  │    audio_data: 去掉 WAV header 后的 PCM 数据,
...
│  │  │  }
│  │
│  ├─ 步骤 10：构建 Task 并提交到 Worker Pool
│  │  ├─ task = {
│  │  │    id: eval_id,
│  │  │    type: "evaluate",
│  │  │    user_id: user_id,
...
│  │
│  ├─ Worker goroutine 从任务队列中拉取 evaluate 任务
│  │  ├─ 调用 EvaluateTaskProcessor.Process(ctx, task)
│  │  └─ 返回评测结果对象 result（包含分数、级别、问题单词等）和 result_key（evaluate:result:{eval_id}）
│  │
│  ├─ 步骤 A1：调用讯飞语音评测 API
│  │  ├─ 更新评测状态：status = "processing", current_stage = "evaluating"
│  │  ├─ 调用 EvaluationProvider.Assess(target_text, audio_data)
│  │  │  ├─ 请求参数：
│  │  │  │  ├─ audio_data（二进制音频数据）
│  │  │  │  ├─ text（标准文本）
│  │  │  │  ├─ language（语言代码）
...
│  │  │  ├─ 异常处理：
│  │  │  │  ├─ 超时 → 重试 3 次（指数退避）
│  │  │  │  ├─ API 认证失败 → 返回错误（status = "failed"）
│  │  │  │  ├─ 音频质量问题 → 返回错误（status = "failed"）
│  │  │  │  ├─ 网络错误 → 重试 3 次
│  │  │  │  └─ API 错误 → 返回错误（status = "failed"）
│  │  │  └─ 返回：评测结果对象，包含：
│  │  │     ├─ total_score（总体得分 0-100）
│  │  │     ├─ accuracy（准确度得分）
│  │  │     ├─ fluency（流利度得分）
│  │  │     ├─ completeness（完整度得分）
│  │  │     ├─ words[]（按单词拆分的得分和详细信息）
│  │  │
│  │  └─ 异常处理：无法获取任何结果 → 返回错误（status = "failed"）
│  │
│  ├─ 步骤 A2：后处理和分析评测结果
│  │  ├─ 验证评测数据（0 ≤ score ≤ 100 等）
│  │  ├─ 计算反馈级别 feedback_level（S/A/B/C）
│  │  ├─ 分析 words 列表，找出问题单词：
│  │  │  ├─ error_words = 筛选(score < 60)
│  │  │  ├─ 找出得分最低的单词 worst_word
│  │  │  └─ 构建 WordDetails 数组（Word, Score, IsProblem）
│  │
│  ├─ 步骤 A3：LLM 生成分级反馈文本
│  │  ├─ 构建 system_prompt 和 user_message（包含 target_text、score、worst_word 等）
│  │  ├─ 调用 LLMProvider.Chat(system_prompt, user_message)
│  │  │  ├─ 异常处理：LLM 调用失败 → 使用 levelTextMap[feedback_level] 作为 fallback
│  │  └─ 得到 feedback_text
│  │
│  ├─ 步骤 A4：TTS 合成反馈音频
│  │  ├─ 调用 TTSProvider.Synthesize(feedback_text)
│  │  │  └─ 异常处理：合成失败 → 返回错误（可只保留文本）
│  │  └─ 得到 feedback_audio（二进制）
│  │
│  ├─ 步骤 A5：条件生成示范音频
│  │  ├─ 如果 feedback_level 为 "A"/"B" 且 worst_word 非空：
│  │  │  ├─ demo_type = "word"
│  │  │  ├─ demo_text = worst_word
│  │  │  └─ 调用 TTSProvider.Synthesize(demo_text) 生成单词示范音频
│  │  ├─ 如果 feedback_level 为 "C"：
│  │  │  ├─ demo_type = "sentence"
│  │  │  ├─ demo_text = target_text
│  │  │  └─ 调用 TTSProvider.Synthesize(target_text) 生成整句示范音频
│  │  └─ 得到 demo_audio_data（可为空）
│  │
│  ├─ 步骤 A6：上传音频到 OSS
│  │  ├─ 生成 eval_id 对应的路径前缀：evaluate/{eval_id}/...
│  │  ├─ 上传 feedback_audio → 得到 feedback_audio_url
│  │  ├─ 如果 demo_audio_data 非空 → 上传 demo 音频 → 得到 demo_audio_url
│  │  └─ 异常处理：上传失败 → 记录日志，但不影响主流程
│  │
│  ├─ 步骤 A7：保存评测结果到数据库
│  │  ├─ 构建 PronunciationEvaluation 记录：
│  │  │  ├─ id = eval_id
│  │  │  ├─ user_id
│  │  │  ├─ target_text
│  │  │  ├─ overall_score / accuracy_score / fluency_score / integrity_score
│  │  │  ├─ feedback_level
│  │  │  ├─ feedback_text
│  │  │  ├─ feedback_audio_url
│  │  │  ├─ problem_words（数组）
│  │  │  ├─ difficulty_level
│  │  │  ├─ demo_sentence_audio_url 或 problem_word_audio_urls（根据 demo_type）
│  │  │  ├─ status = "completed"
│  │  │  └─ created_at
...
│  │  └─ 异常点：格式不合法 → 返回 400
│  │
│  ├─ 步骤 5：从 Redis 缓存查询评测任务状态
│  │  ├─ 调用 Cache.GetTaskStatus(task_id)
│  │  ├─ 返回：任务对象（包含 status、result_key、user_id 等）
│  │  └─ 异常点：
│  │     ├─ Redis 连接失败 → 降级到数据库查询
│  │     └─ 评测不存在于缓存 → 继续步骤 6
│  │
...
│  │  └─ 如果存在于缓存：
│  │     └─ 检查 task.user_id == 当前登录用户的 user_id，否则返回 403
│  │
│  ├─ 步骤 7：根据评测状态返回不同的响应
│  │  │
